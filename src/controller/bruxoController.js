import * as BruxoModel from '../model/bruxoModel.js';


export const getAllBruxos = async (req, res) => {
  try {
    ...
    if (!bruxos || bruxos.length === 0) {
      res.status(404).json({
        total: bruxos.length,
        mensagem: 'Não há bruxos cadastrados',
        bruxos: []
      });
    }
...
export const atualizar = async (req, res) => {
  try {
    ...
    const bruxoExiste = await BruxoModdel.findById(id);
...
export const apagar = async (req, res) => {
  try {
    ...
    res.status(200).json({
      mensagem: 'Bruxo removido com sucessoo',
